...
class NeuralNetwork():
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate=0.1):
        ...
        self.weights_input_hidden = np.random.uniform(
            -1, 1, size=(hidden_nodes, input_nodes))
        self.weights_hidden_output = np.random.uniform(
            -1, 1, size=(output_nodes, hidden_nodes))
    ...
    def backpropagation(self, input_v, target_v):
        ...
        self.weights_hidden_output = np.add(
            self.weights_hidden_output, weights_ho_deltas)
        self.bias_output = np.add(self.bias_output, gradient)

        transpose_weights_hidden_output = np.transpose(
        ...
        self.weights_input_hidden = np.add(
            self.weights_input_hidden, weights_ih_deltas)
